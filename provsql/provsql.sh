...
if [[ -z ${CONTAINERS} ]]; then
	...
	echo	'latest ubuntu version is supposed to be:'
	echo	${RELEVANT_YEAR}.${RELEVANT_MONTH}
	echo	'udocker run --containerauth --publish=2345:5432 --volume=`pwd`:/scripts psql bash /scripts/psql.sh'
	echo	'leave the terminal open. connect from another terminal on the same server with: '
	echo	'conda create -n check_conn / conda create --prefix=/path/to/env/check_conn'
	echo	'conda activate check_conn'
	echo	'conda install psycopg2'
	echo 	'python'
	echo	'import psycopg2'
	echo	'conn = psycopg2.connect("dbname=postgres user=postgres password=123 host=127.0.0.1 port=2345")'
	echo	'print(conn)'

	udocker pull ubuntu:latest > /dev/null 2>&1
	udocker create --name=psql ubuntu:latest
	udocker setup --execmode=P2 psql
	udocker run --containerauth --publish=2345:5432 --volume=`pwd`:/scripts psql bash /scripts/psql.sh
fi